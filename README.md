...
**לפני (הגרסה המקורית):** גרסה 1.0 — דיבורית בלוטוס למחשב Windows: שיחות (HFP), יומן שיחות, תא קולי, הודעות DTMF, בייביסיטר, בית חכם, אנשי קשר — ממשק PySide6 עברי/אנגלי.

**מה שופר בגרסה הנוכחית:**
- הקוד כולו עבר ריפקטורינג ושכתוב — פיצול מודולרי מלא ל-`app/core` (מנועים), `app/pages` (מסכים) ו-`app/widgets` (רכיבים)
- נוספו בדיקות אוטומטיות ללוגיקה המרכזית (`tests/test_core_logic.py`)
- נוסף קובץ `SmartKosherPhone.spec` אחיד לבניית EXE חוזרת
- נבנה ופורסם קובץ EXE מוכן (SmartKosherPhone.exe, ~356MB) ב-`dist/` — הורדה והרצה ללא Python
- נוסף רישיון + README מפורט בעברית עם באנר, הוראות הרצה וקישור הורדה
